...
class FakeExecutor implements ISqlCommandExecutor {
	...
	async execute<T>(command: string, args: Record<string, unknown> = {}, options: { allowVoid?: boolean } = {}): Promise<T> {
...
test('open forwards profile and secret to the backend', async () => {
	...
	const payload = executor.calls[0].args as { secret: { password?: string } };
