...
pub fn run() {
    ...
    let app = register_webdriver_for_test(tauri::Builder::default())
        ...
        .on_page_load(|webview, _payload| {
            inject_webdriver_nonce_for_test(webview);
        ...
        .setup(|app| {
            let app_data = commands::os::resolve_app_data_dir(app.handle())
...
#[cfg(all(feature = "webdriver", debug_assertions))]
fn inject_webdriver_nonce_for_test<R: tauri::Runtime>(webview: &tauri::Webview<R>) {
    if std::env::var_os(tauri_plugin_wdio_webdriver::PORT_ENV_VAR).is_none() {
        return;
    }
    let Some(nonce) =
        std::env::var_os("NYALA_WEBDRIVER_RUN_NONCE").and_then(|value| value.into_string().ok())
    else {
        return;
    };
    let Ok(encoded_nonce) = serde_json::to_string(&nonce) else {
        return;
    };
    let script = format!(
        "Object.defineProperty(globalThis, '__nyalaWebdriverRunNonce', {{ value: {encoded_nonce}, configurable: false, enumerable: false, writable: false }});"
    );
    if let Err(error) = webview.eval(script) {
        log::warn!("WebDriver ownership nonce could not be injected: {error}");
    }
}

#[cfg(not(all(feature = "webdriver", debug_assertions)))]
fn inject_webdriver_nonce_for_test<R: tauri::Runtime>(_webview: &tauri::Webview<R>) {}
